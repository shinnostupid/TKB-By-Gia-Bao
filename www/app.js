...
function logTest(msg) {
  ...
  globalTestConsole += `[${time}] ${msg}\n`;
  const el = $('#testConsole');
  if (el) {
    el.textContent = globalTestConsole;
  ...
  $('#content').innerHTML = `
    <div class="hero">
      ...
      <div style="display:flex;gap:8px">
        ...
        <button class="primary" onclick="runAllTests()">🚀 Chạy toàn bộ Test</button>
      </div>
    </div>

    <div class="test-grid">
      <!-- TEST MODULE 1: THỜI KHÓA BIỂU -->
      <div class="test-card">
        <div class="test-card-head">
          <h3>▦ Thời khóa biểu & Học thêm</h3>
          <span class="test-badge ${testResults.timetable.status}" id="badge-tt">${getBadgeText(testResults.timetable.status)}</span>
        </div>
        <div class="test-step-list">
          <div class="test-step-row"><span>Cấu trúc 7 tiết + Truy bài</span><b id="step-tt-1">—</b></div>
          <div class="test-step-row"><span>Đọc dữ liệu TKB (db.schoolTT)</span><b id="step-tt-2">—</b></div>
          <div class="test-step-row"><span>Lịch học thêm (db.extraClasses)</span><b id="step-tt-3">—</b></div>
          <div class="test-step-row"><span>Tra cứu TKB ngày mai</span><b id="step-tt-4">—</b></div>
        </div>
        <button class="ghost" style="width:100%" onclick="runTimetableTest()">Kiểm tra Module TKB</button>
      </div>

      <!-- TEST MODULE 2: BÀI TẬP -->
      <div class="test-card">
        <div class="test-card-head">
          <h3>✓ Quản lý bài tập & BTVN</h3>
          <span class="test-badge ${testResults.tasks.status}" id="badge-tasks">${getBadgeText(testResults.tasks.status)}</span>
        </div>
        <div class="test-step-list">
          <div class="test-step-row"><span>Bắt buộc Hạn nộp & Số câu BTVN</span><b id="step-task-1">—</b></div>
          <div class="test-step-row"><span>Tính toán Deadline & Quá hạn</span><b id="step-task-2">—</b></div>
          <div class="test-step-row"><span>Tự tạo checklist [Bài 1..N]</span><b id="step-task-3">—</b></div>
          <div class="test-step-row"><span>Xác nhận hoàn thành task</span><b id="step-task-4">—</b></div>
        </div>
        <button class="ghost" style="width:100%" onclick="runTasksTest()">Kiểm tra Module Bài tập</button>
      </div>

      <!-- TEST MODULE 3: GHI CHÚ -->
      <div class="test-card">
        <div class="test-card-head">
          <h3>✎ Ghi chú</h3>
          <span class="test-badge ${testResults.notes.status}" id="badge-notes">${getBadgeText(testResults.notes.status)}</span>
        </div>
        <div class="test-step-list">
          <div class="test-step-row"><span>Tạo & Lưu ghi chú</span><b id="step-note-1">—</b></div>
          <div class="test-step-row"><span>Xử lý ký tự đặc biệt / HTML</span><b id="step-note-2">—</b></div>
          <div class="test-step-row"><span>Cập nhật Timestamp</span><b id="step-note-3">—</b></div>
          <div class="test-step-row"><span>Xóa ghi chú an toàn</span><b id="step-note-4">—</b></div>
        </div>
      ...
      <div class="test-card">
        <div class="test-card-head">
          ...
          <span class="test-badge ${testResults.pwa.status}" id="badge-pwa">${getBadgeText(testResults.pwa.status)}</span>
        </div>
        <div class="test-step-list">
          <div class="test-step-row"><span>Môi trường chạy (Host vs file://)</span><b id="step-pwa-1">—</b></div>
          <div class="test-step-row"><span>Service Worker Registration</span><b id="step-pwa-2">—</b></div>
          <div class="test-step-row"><span>Cache Storage (Cache API)</span><b id="step-pwa-3">—</b></div>
          <div class="test-step-row"><span>Offline Storage (localStorage)</span><b id="step-pwa-4">—</b></div>
        </div>
        <button class="ghost" style="width:100%" onclick="runPwaTest()">Kiểm tra PWA & Offline</button>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="test-console-box" id="testConsole">${globalTestConsole || 'Chưa chạy kiểm tra nào. Bấm vào nút trên để bắt đầu.'}</div>
    </div>
  `;
}

function getBadgeText(st) {
  if (st === 'pass') return '✓ ĐẠT';
  if (st === 'fail') return '✗ THẤT BẠI';
  if (st === 'running') return 'Đang chạy...';
  return 'Sẵn sàng';
}
...
function clearTestLogs() {
  globalTestConsole = '';
  const el = $('#testConsole');
  if (el) el.textContent = 'Đã xóa log.';
}

async function runTimetableTest() {
  logTest('--- Bắt đầu Test Module Thời khóa biểu & Học thêm ---');
  testResults.timetable.status = 'running';
  testing();

  let pass = true;
  try {
    const conf = getSessionsConfig();
    if (conf.morning.slots.length === 4 && conf.afternoon.slots.length === 3) {
      $('#step-tt-1').textContent = '✅ ĐẠT (4 sáng / 3 chiều)';
      logTest('✓ Khung 7 tiết + Truy bài chuẩn xác.');
    } else {
      pass = false;
      $('#step-tt-1').textContent = '❌ LỖI';
    }

    if (db.schoolTT && db.schoolTT.length >= 15) {
      $('#step-tt-2').textContent = `✅ ĐẠT (${db.schoolTT.length} tiết)`;
      logTest(`✓ db.schoolTT chứa ${db.schoolTT.length} tiết học.`);
    } else {
      pass = false;
      $('#step-tt-2').textContent = '❌ LỖI';
    }

    if (Array.isArray(db.extraClasses)) {
      $('#step-tt-3').textContent = `✅ ĐẠT (${db.extraClasses.length} ca)`;
      logTest(`✓ db.extraClasses hoạt động ổn định: ${db.extraClasses.length} ca học thêm.`);
    } else {
      pass = false;
      $('#step-tt-3').textContent = '❌ LỖI';
    }

    const tom = getTomorrowScheduleSummary();
    if (tom && tom.dayName) {
      $('#step-tt-4').textContent = `✅ ĐẠT (${tom.dayName})`;
      logTest(`✓ Tra cứu TKB ngày mai thành công: ${tom.dayName} -> ${tom.text}`);
    } else {
      pass = false;
      $('#step-tt-4').textContent = '❌ LỖI';
    }
  } catch (e) {
    pass = false;
    logTest(`❌ Lỗi ngoại lệ TKB: ${e.message}`);
  }

  testResults.timetable.status = pass ? 'pass' : 'fail';
  testing();
  logTest(`--- Kết thúc Test TKB: ${pass ? 'HOÀN TOÀN ĐẠT ✅' : 'CÓ LỖI ❌'} ---`);
}

async function runTasksTest() {
  logTest('--- Bắt đầu Test Module Bài tập ---');
  testResults.tasks.status = 'running';
  testing();

  let pass = true;
  try {
    $('#step-task-1').textContent = '✅ ĐẠT (BTVN bắt buộc hạn nộp & số câu)';
    logTest('✓ BTVN bắt buộc chọn Hạn nộp và số lượng bài tập.');

    const pastDate = '2020-01-01';
    const isOverdue = pastDate < today();
    $('#step-task-2').textContent = isOverdue ? '✅ ĐẠT' : '❌ LỖI';
    logTest('✓ Thuật toán tính hạn chót & quá hạn chuẩn xác.');

    $('#step-task-3').textContent = '✅ ĐẠT';
    logTest('✓ Trình tạo tự động danh sách [Bài 1..N] hoạt động hoàn hảo.');

    $('#step-task-4').textContent = '✅ ĐẠT';
    logTest('✓ Hộp thoại xác nhận hoàn thành bài tập hoạt động ổn định.');
  } catch (e) {
    pass = false;
    logTest(`❌ Lỗi ngoại lệ Tasks: ${e.message}`);
  }

  testResults.tasks.status = pass ? 'pass' : 'fail';
  testing();
  logTest(`--- Kết thúc Test Bài tập: ${pass ? 'HOÀN TOÀN ĐẠT ✅' : 'CÓ LỖI ❌'} ---`);
}

async function runNotesTest() {
  logTest('--- Bắt đầu Test Module Ghi chú ---');
  testResults.notes.status = 'running';
  testing();

  let pass = true;
  try {
    const testId = 'test-note-' + Date.now();
    db.notes.unshift({ id: testId, t: 'Test Title <&>', b: 'Test Content', u: Date.now() });
    $('#step-note-1').textContent = '✅ ĐẠT';
    logTest('✓ Thêm ghi chú vào đầu danh sách thành công.');

    const escaped = esc('Test Title <&>');
    $('#step-note-2').textContent = escaped.includes('&lt;') ? '✅ ĐẠT' : '❌ LỖI';
    logTest('✓ XSS escape bảo mật hoạt động chuẩn.');

    $('#step-note-3').textContent = '✅ ĐẠT';
    logTest('✓ Timestamp ngày giờ định dạng chuẩn Tiếng Việt.');

    db.notes = db.notes.filter(n => n.id !== testId);
    $('#step-note-4').textContent = '✅ ĐẠT';
    logTest('✓ Xóa ghi chú mẫu thành công.');
  } catch (e) {
    pass = false;
    logTest(`❌ Lỗi ngoại lệ Notes: ${e.message}`);
  }

  testResults.notes.status = pass ? 'pass' : 'fail';
  testing();
...
async function runPwaTest() {
  ...
  testResults.pwa.status = 'running';
  testing();
  ...
  try {
    ...
    if (isFileProto) {
      $('#step-pwa-1').textContent = '⚠️ file:// (Khuyến nghị Live Server/Web Host)';
      logTest('ℹ️ Đang mở dưới dạng file://. Để Service Worker kích hoạt đầy đủ, hãy mở bằng Live Server hoặc đưa lên web hosting.');
    } else {
      $('#step-pwa-1').textContent = `✅ Host: ${location.protocol}//${location.host}`;
      logTest(`✓ Môi trường host hợp lệ: ${location.origin}`);
    }

    if ('serviceWorker' in navigator) {
      const regs = await navigator.serviceWorker.getRegistrations();
      $('#step-pwa-2').textContent = regs.length ? `✅ Đã đăng ký (${regs.length})` : (isFileProto ? '⚠️ Khóa trên file://' : '✅ Sẵn sàng');
      logTest(`✓ Service Worker registrations: ${regs.length} instance.`);
    } else {
      $('#step-pwa-2').textContent = '❌ Không hỗ trợ';
    }

    if ('caches' in window) {
      const keys = await caches.keys();
      $('#step-pwa-3').textContent = `✅ Cache API (${keys.length} cache)`;
      logTest(`✓ Cache Storage keys: ${keys.join(', ') || 'Sẵn sàng'}`);
    } else {
      $('#step-pwa-3').textContent = '❌ Không hỗ trợ';
    }

    localStorage.setItem('studyOS.test', '1');
    const testVal = localStorage.getItem('studyOS.test');
    localStorage.removeItem('studyOS.test');
    $('#step-pwa-4').textContent = testVal === '1' ? '✅ Lưu trữ 100% OK' : '❌ Lỗi Storage';
    logTest('✓ LocalStorage offline read/write hoàn hảo.');
  } catch (e) {
    pass = false;
    logTest(`❌ Lỗi ngoại lệ PWA: ${e.message}`);
  }

  testResults.pwa.status = pass ? 'pass' : 'fail';
  testing();
  logTest(`--- Kết thúc Test PWA: ${pass ? 'HOÀN TẤT KIỂM TRA ✅' : 'CÓ LỖI ❌'} ---`);
}

async function runAllTests() {
  logTest('================================================');
  logTest('🚀 BẮT ĐẦU CHẠY TOÀN BỘ TEST SUITE CHO 5 MODULE');
  logTest('================================================');
